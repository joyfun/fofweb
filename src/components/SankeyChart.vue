  ...
  computed: {
      ...
      get() {
        ...
        this.finalData.forEach((element) => {
          ...
          if (cls != 'FOF') {
            ...
            nlinks.push({
              source: this.showFundName(element.code),
              target: this.showFundName(element.b_code),
              value: Math.floor(element['marketval'] / 10000)
            })
          }
        })
        ...
        for (var key in ctype) {
          let cds = key.split('_')
          nlinks.push({
            source: this.showFundName(cds[0]),
            target: this.showFundName(cds[1]),
            value: Math.floor(ctype[key] / 10000)
          })
    ...
    nodes: {
      get() {
        ...
        return [
          ...this.sysparam.FOF.map((row) => row['code']),
          ...this.sysparam.class_type
            .filter((row) => row['code'] != '主观' && row['code'] != 'FOF')
            .map((row) => row['code'])
        ].map((arow) => {
          return { 'name': this.showFundName(arow) }
        })
