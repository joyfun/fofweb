...
export default {
  ...
  data() {
    return {
      ...
      keygrps: [
        { name: '收益', cols: ['yr', 'median_hyr', 'profit'], gap: [-0.2] },
        { name: '波动', cols: ['vola', 'risk'], gap: [0, 1] },
        { name: '风险', cols: ['nowdrop', 'adj_risk'], gap: [-1, 0] },
        { name: '收益风险比', cols: ['calm', 'adj_calmar'], gap: [-1, 10] },
        { name: '收益波动', cols: ['yr', 'vola'], gap: [-0.2] }
      ],
      showkey: '收益',
  ...
  methods: {
    ...
    getChartData() {
      this.$axios
        .get('/fof/hisstat', {
          params: { 'code': this.code, 'range': this.range }
        })
        .then((response) => {
