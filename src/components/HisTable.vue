...
export default {
  ...
  methods: {
    ...
    uploadFile(opts) {
      ...
      $table.readFile(opts).then((params) => {
        ...
        reader.onload = function (e) {
          ...
          that.records = XLSX.utils.sheet_to_json(
            workbook.Sheets[workbook.SheetNames[0]],
            { raw: false, dateNF: 'yyyymmdd' }
          )
          that.impcols = []
    ...
    saveHis() {
      const $table = this.$refs.hisTable
      let { insertRecords, removeRecords, updateRecords } =
        $table.getRecordset()
    ...
    confirmData() {
      var $this = this
      axis
        .get('/fof/confirm', { params: { code: this.code } }) //axis后面的.get可以省略；
        .then((response) => {
          if (response.data.status == 'success') $this.getTable(this.code)
        })
    },
    loadModel(file) {
      var that = this
      ...
      const insert = DB.prepare(
        'replace into fund_val(code,date ,sumval ) VALUES (? ,? ,?)'
      )
      ...
      reader.onload = function (e) {
        ...
        let rst = XLSX.utils
          ...
          .map((row) => {
            return [that.code, row['date'].replaceAll('-', ''), row['累计净值']]
          })
        insertMany(rst)
        that.$message({
          message: '成功导入' + rst.length + '条数据',
    ...
    downloadData() {
      ...
      var nst = XLSX.utils.json_to_sheet(
        ...
        { header: ['date', '累计净值'], skipHeader: false }
      )
      XLSX.utils.book_append_sheet(nwb, nst, 'Sheet1')
      var wbout = XLSX.write(nwb, {
        bookType: 'xlsx',
        bookSST: true,
        type: 'array'
      })
    ...
    getTable(code) {
      if (code && code.length > 2) {
        ...
        if (this.$isElectron) {
          ...
          axis
            .get('/fof/his', { params: param }) //axis后面的.get可以省略；
            .then((response) => {
              $this.tableData = []
