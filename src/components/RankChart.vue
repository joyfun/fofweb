      ...
      this.$axios
        ...
        .then((response) => {
          ...
          for (let col in response.data.columns) {
            ...
            let rval = response.data.data.map((row) => {
              if (row[col] == null) {
                return null
              }
              return row[col] * 100
          ...
          let yrago = this.$moment(option.xAxis[0].data[dlen - 1])
            .add(-1, 'y')
            .format('YYYYMMDD')
          let yrago2 = this.$moment(option.xAxis[0].data[dlen - 1])
            .add(-2, 'y')
            .format('YYYYMMDD')
            ...
            areas: [
              {
                brushType: 'lineX',
                coordRange: [
                  option.xAxis[0].data[sidx],
                  option.xAxis[0].data[dlen - 1]
                ],
                xAxisIndex: 0
              },
              {
