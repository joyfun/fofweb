  ...
  methods: {
          ...
          if (this.collen < 5) {
            if (cname == '华道多策略FOF' && this.cb) {
              var param = {
                'name': cname,
                'lrate': maxdrop,
                'start': this.raw_data.date[mhidx],
                'end': this.raw_data.date[midx]
              }
    ...
    initChart() {
      ...
      if (this.echart) {
        ...
        this.axisOption.toolbox.feature.mydayyear2.onclick = () => {
          var vday = $this
            .$moment($this.max_date)
            .add(-2, "y")
            .format("YYYYMMDD");
        ...
        this.axisOption.toolbox.feature.mydayyear3.onclick = () => {
          var vday = $this
            .$moment($this.max_date)
            .add(-3, "y")
            .format("YYYYMMDD");
      ...
      if (this.raw_data['wts']) {
        var cols = this.raw_data['columns'].filter(
          (cn) => !(cn.endsWith('指数') || cn.endsWith('_超额'))
        )
    ...
    combineGroup(validx, wts) {
      ...
      this.raw_data['虚拟组合'] = Array.apply(null, Array(dlen)).map(function (
        x,
        i
      ) {
        return null
      ...
      if (!wts) {
        wts = Array.apply(null, Array(len(validx))).map(function (x, i) {
          return 1
        })
      }
      var wsum = 0
