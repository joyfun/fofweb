...
export default {
  ...
  methods: {
    ...
    jumpToSource(row) {
      ...
      if (row.type == 'GeShang') {
        url = 'https://www.licai.com/simu/product/' + scode
      } else if (row.type == 'SiMuWang') {
        url = 'https://dc.simuwang.com/product/' + scode + '.html'
      } else if (row.type == 'ZhaoYang') {
        url = 'https://fof.12345fund.com/details/0-1?id=' + scode
      }
      if (url) {
        window.open(url)
      }
    ...
    batchConfirm() {
      ...
      //  var $this=this
      this.$axios
        .get('/fof/confirmBatch', { params: { codes: sels.join() } }) //axis后面的.get可以省略；
        .then((response) => {
          if (response.data.status == 'success') this.getList()
        })
    },
