  ...
  <div id="tableContainer" ref="tableContainer" style="height: 100%">
    <el-row>
      <el-col :span="16">
        ...
        >
          <vxe-column title="类型" sortable field="type" show-overflow-tooltip>
            <template #header> {{ data_time }}<br />类型 </template>
            <template slot-scope="scope">
              {{
                scope.row['rank_type'] == '指增'
                  ? '指增超额'
                  : scope.row['rank_type']
              }}
            </template>
          ...
          <vxe-colgroup title="收益(年化)" align="center">
            ...
            >
              <template #default="{ row }">
                <span>{{
                  $tools.formatMoney(row.yeaily_return * 100, 2)
                }}</span>
              </template>
              <template #edit="{ row }">
                <vxe-input
                  type="number"
                  v-model="row['yeaily_return']"
                  class="myinput"
                ></vxe-input>
              </template>
            </vxe-column>
            ...
            >
              <template #default="{ row }">
                <span>{{
                  $tools.formatMoney(row.yeaily_return_w * 100, 2)
                }}</span>
              </template>
              <template #edit="{ row }">
                <vxe-input
                  type="number"
                  v-model="row['yeaily_return_w']"
                  class="myinput"
                ></vxe-input>
              </template>
            </vxe-column>
          ...
          <vxe-colgroup title="资金" align="center">
            ...
            >
              ...
              <template #default="{ row }">
                <span>{{ $tools.formatMoney(row.adj, 0) }}</span>
              </template>
              <template #edit="{ row }">
                <vxe-input
                  type="number"
        ...
        -->
            ...
            >
              <template slot-scope="scope">{{
                $tools.formatMoney(
                  ((scope.row.marketval + parseFloat(scope.row.adj) * 10000) *
                    100) /
                    total,
                  2
                )
    ...
    <el-row>
      <el-col :span="16">
        <vxe-table
          ...
          :sort-config="{
            trigger: 'cell',
            defaultSort: { field: 'marketval', order: 'desc' },
  ...
  methods: {
    showPercentComp(row) {
      ...
      if (Math.abs(this.ptotal - this.total) > 0) {
        ret =
          this.$tools.formatMoney(
            ((row.marketval + parseFloat(row.adj) * 10000) * 100) / this.total,
            2
          ) +
          '/' +
          ret
    ...
    genPercent(fcode) {
      ...
      let subdict = {
        '中证500': { 'marketval': 0, 'adj': 0 },
        '指增': { 'marketval': 0, 'adj': 0 },
        'cta1': { 'marketval': 0, 'adj': 0 },
        'cta0': { 'marketval': 0, 'adj': 0 },
        ...
        .forEach((srow, idx) => {
          ...
          if (hdict) {
            for (let skey in hdict) {
              ...
              subdict[skey]['adj'] +=
                (nrt * (hdict[skey]['marketval'] + hdict[skey]['adj'] * 10000) -
                  hdict[skey]['marketval'] * rt) /
                10000
            }
          }
      ...
      this.actionData
        .filter(
          (row) =>
            (row['code'] == fcode || row['b_code'] == fcode) &&
            row['status'] != '结束'
        )
        .map((row) => {
      ...
      let ptotal = 0
      let atotal = 0
      for (let k in subdict) {
        if (k != '中证500') {
          ptotal += subdict[k]['marketval']
          atotal = atotal + subdict[k]['marketval'] + subdict[k]['adj'] * 10000
        }
      }
      this.adjsum[fcode] = atotal
      this.subsum[fcode] = ptotal
      for (let k in subdict) {
        subdict[k]['prate'] = subdict[k]['marketval'] / ptotal
        subdict[k]['arate'] =
          (subdict[k]['marketval'] + subdict[k]['adj'] * 10000) / atotal
      }
    ...
    genHoldingData() {
      ...
      for (let atype in this.sumdict) {
        if (atype && 'null' != atype) {
          let row = {
            'adj': this.sumdict[atype]['adj'],
            'rank_type': atype,
            'marketval': this.sumdict[atype]['marketval'],
            'yeaily_return_hyr': this.sumdict[atype]['yeaily_return_hyr'] / 2,
            'yeaily_return_1yr': this.sumdict[atype]['yeaily_return_1yr'],
            'yeaily_return_2yr': this.sumdict[atype]['yeaily_return_2yr'] * 2
          }
          let tkey = atype
          if (atype == '混合') {
            tkey = 'aas'
          ...
          if (row['rank_type'] == '中证500') {
            if (this.performs['000905.SH']) {
              ...
              // }
              row['yeaily_return_hyr'] +=
                this.performs['000905.SH']['yeaily_return_hyr'] / 2
              row['yeaily_return_1yr'] +=
                this.performs['000905.SH']['yeaily_return_1yr']
              row['yeaily_return_2yr'] +=
                this.performs['000905.SH']['yeaily_return_2yr'] * 2
    ...
    changeHoldingtype(mcode) {
      ...
      this.subholding = this.addPerformance(
        this.holdings.filter((row) => !row['b_code'].startsWith('SUBJECT'))
      )
      this.genHoldingData()
    },
    getHoldingType(code) {
    ...
    getPerform(codes) {
      ...
      console.log(acode)
      this.$axios
        .get('/fof/recentPerform', {
          params: { 'code': acode.join(','), 'key': 'yeaily_return' }
        }) //axis后面的.get可以省略；
        .then((response) => {
    ...
    addPerformance(holds) {
      ...
      holds.map((row) => {
        if (row['rank_type'] != 'FOF') {
          for (let key of ['hyr', '1yr', '2yr']) {
            this.sumdict[row['rank_type']]['yeaily_return_' + key] +=
              row['percent'] * row['yeaily_return_' + key]
          }
        }
      })
      console.log(holds)
    ...
    refreshSubPie() {
      ...
      this.holdings.map((row) => {
        if (
          row['level'] &&
          (row['rank_type'] == this.clicktype || !this.clicktype)
        ) {
    ...
    adjamount() {
      ...
      this.pieData.series.data = this.holdingData.map((row) => {
        if (row['rank_type'] != '中证500') {
          if (row['adj'] == '') {
            row['adj'] = 0
          }
          ...
          return {
            'name': row['rank_type'],
            'value': row['marketval'] + row['adj'] * 10000
          }
        }
        return null
      })
    ...
    calcCast() {
      if (this.performs['000905.SH']) {
        ...
        this.holdingData.map((row) => {
          let pct =
            (row['marketval'] + parseFloat(row['adj']) * 10000) / this.total
        ...
        if (cmap['中证500']) {
          prev.dd = this.$tools.formatMoney(
            ((cmap['中证500']['dd'] *
              (cmap['中证500']['marketval'] + cmap['中证500']['adj'] * 10000)) /
              this.total -
              (prev.vola * 0.6) / 100) *
              100,
            2
          )
          nprev.dd = this.$tools.formatMoney(
            ((this.indexData['dd'] * cmap['中证500']['marketval']) /
              this.ptotal -
              (nprev.vola * 0.6) / 100) *
              100,
            2
        ...
        return [
          [
            ...
            nprev.yr_w,
            nprev.dd,
            (this.ptotal / 10000).toFixed(0),
            '',
            '',
            '近期',
            this.$tools.formatMoney(
              (this.performs[this.cur_fof]['yeaily_return_hyr'] * 100) / 2,
              2
            ),
            this.$tools.formatMoney(
              this.performs[this.cur_fof]['yeaily_return_1yr'] * 100,
              2
            ),
            this.$tools.formatMoney(
              this.performs[this.cur_fof]['yeaily_return_2yr'] * 200,
              2
            ),
            nprev.vola
          ],
          [
            ...
            prev.yr_w,
            prev.dd,
            (this.total / 10000).toFixed(0),
            this.$tools.formatMoney(sadj, 0),
            '',
            '',
            '',
            '',
