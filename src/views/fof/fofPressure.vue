  ...
  <div>
    <div class="block" style="display: flex; justify-content: space-between">
      <div style="width: 800px">
        ...
        预测中证500<el-input-number
          style="width: 200px"
          @change="changeNum"
          placeholder="中证500"
          v-model="zz500"
        ></el-input-number>
        预测中证回撤<el-input-number
          style="width: 200px"
          placeholder="中证500回撤"
          v-model="zz500back"
        ></el-input-number>
    ...
    >
      <el-table-column width="120" label="类型" show-overflow-tooltip>
        <template slot-scope="scope">{{ scope.row['类型'] }}</template>
      </el-table-column>
      <el-table-column width="120" label="预期收益" show-overflow-tooltip>
        <template slot-scope="scope">
          ...
          >
            {{
              showResult(scope.row['过去3年年均收益'] / scope.row['count'], 100)
            }}</span
          >
        </template>
      </el-table-column>
      <el-table-column width="120" label="预期回撤" show-overflow-tooltip>
        <template slot-scope="scope">
          <span
            :style="
              'text-align:right;color:' +
              (scope.row['预期回撤'] >= 0 ? 'red' : 'green')
            "
          >
      ...
      <el-table-column width="120" label="悲观收益" show-overflow-tooltip>
        <template slot-scope="scope">
          <span
            :style="
              'text-align:right;color:' +
              (scope.row['悲观收益'] >= 0 ? 'red' : 'green')
            "
          >
      ...
      <el-table-column width="160" label="当前额度" show-overflow-tooltip>
        <template slot-scope="scope">
          <span
            :style="
              'text-align:right;color:' +
              (scope.row['当前额度'] >= 0 ? 'red' : 'green')
            "
          >
            {{
              showResult(scope.row['当前额度'] + scope.row.change * 10000)
            }}</span
          >
        </template>
      </el-table-column>
      ...
      <el-table-column width="80" label="权重%" show-overflow-tooltip>
        <template slot-scope="scope">
          {{
            showResult(
              (scope.row['当前额度'] + scope.row.change * 10000) / all,
              100
            )
          }}
        </template>
      ...
      <el-table-column width="180" label="调整额度(万)" show-overflow-tooltip>
        <template slot-scope="scope">
          <el-input-number
            style="width: 150px"
            @input="changeNum"
      ...
      <el-table-column width="80" label="预期收益贡献" show-overflow-tooltip>
        <template slot-scope="scope">
          <!-- <span :style="'text-align:right;color:'+(scope.row['当前额度']>=0?'red':'green') " > -->
          {{
            showResult(
              ((scope.row['当前额度'] + scope.row.change * 10000) / all) *
                (scope.row['过去3年年均收益'] / scope.row.count +
                  (scope.row['类型'] == '超额') * zz500)
            )
          }}
          <!-- </span> -->
        </template>
      </el-table-column>
      <el-table-column width="100" label="悲观收益" show-overflow-tooltip>
        <template slot-scope="scope">
          {{
            showResult(
              ((scope.row['当前额度'] + scope.row.change * 10000) / all) *
                (scope.row['悲观收益'] + (scope.row['类型'] == '超额') * zz500)
            )
          }}
        </template>
      </el-table-column>
      <el-table-column width="80" label="预期回撤" show-overflow-tooltip>
        <template slot-scope="scope">
          {{
            showResult(
              ((scope.row['当前额度'] + scope.row.change * 10000) / all) *
                (scope.row['预期回撤'] / scope.row.count +
                  (scope.row['类型'] == '超额') * zz500back)
            )
          }}
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <span
            :style="
              'text-align:right;color:' +
              (scope.row['2018'] >= 0 ? 'red' : 'green')
            "
            >{{ showResult(scope.row['2018']) }}</span
          ></template
        >
      </el-table-column>
  ...
  methods: {
    ...
    changeNum(nval) {
      ...
      for (var i in this.pressdata) {
        this.expectret =
          this.expectret +
          ((this.pressdata[i]['当前额度'] +
            this.pressdata[i]['change'] * 10000) /
            this.all) *
            (this.pressdata[i]['过去3年年均收益'] / this.pressdata[i].count +
              (this.pressdata[i]['类型'] == '超额') * this.zz500)
      }
    ...
    getList() {
      axis
        ...
        .then((response) => {
          ...
          var that = this
          var dict = {}
          var minReduce = function (a, b) {
            return that.$tools.isNull(b) || b > a ? a : b
          }
          ...
          for (var idx in this.tableData) {
            ...
            row['low'] = [row['2023'], row['2022'], row['2021']].reduce(
              minReduce,
              Number.MAX_VALUE
            )
            if (dict[class_type]) {
              ...
              aclas = {
                ...
                '类型': class_type,
                'count': aclas['count'] + 1,
                '悲观收益': [aclas['悲观收益'], row['low']].reduce(
                  minReduce,
                  Number.MAX_VALUE
                ),
                '当前额度': aclas['当前额度'] + row['份额'] * row['当前净值'],
                '预期回撤': aclas['预期回撤'] + row['最大回撤'],
                '过去3年年均收益':
                  aclas['过去3年年均收益'] + row['过去3年年均收益']
              }
              ...
              dict[class_type] = {
                ...
                'count': 1,
                '悲观收益': row['low'],
                '当前额度': row['份额'] * row['当前净值'],
                '过去3年年均收益': row['过去3年年均收益'],
                '预期回撤': row['最大回撤'],
                'change': 0
              }
            }
          ...
          for (var cls in dict) {
            if (dict[cls]['类型'] != '指增') ret.push(dict[cls])
          }
          this.pressdata = ret
