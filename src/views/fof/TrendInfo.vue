      ...
      >
       ...
       <a href="javascript:;" @click="showHis(row)">{{ showFundName(row.code)}}</a>
         ...
         <vxe-column  sortable  title="信息"  width="80" align="center" field="level"  >
          ...
          <vxe-colgroup :key="rg" v-for="rg of ['hydata','y1data','y2data','y3data']" :title="rg+'指标数据'" align="center">
         ...
         <vxe-column :key="af" :width="48" sortable v-for="af of ['yeaily_return', 'sharpe', 'calmar', 'dd', 'volatility']"  :title="af" :field="af"  >

            <!-- <template v-else-if="['dd_ratio'].indexOf(af)>-1" #default="{ row }">
              <span :class="'rank_text_color'+Math.floor(row[af]/25)">{{row[af]}}</span>
            </template> -->
            <template  #default="{ row }">
              <span >{{$tools.formatMoney(row[rg+'_'+af],2)}}</span>
            </template>
          </vxe-column>
          </vxe-colgroup>
