    ...
    >
      ...
      >
        ...
        <template #default="{ row }">
          ...
          <!-- <vxe-button  @click.native.prevent="addCart(row)" type="text" status="primary" size="small" ><i class="iconfont icon-mairu" ></i></vxe-button> -->
          &nbsp;<vxe-button
            @click.native.prevent="oneKeyBuy(row)"
            type="text"
            status="primary"
            size="small"
...
export default {
  ...
  methods: {
    ...
    cellClassBg({ row, rowIndex, column, columnIndex }) {
      if (
        ...
        if (this.statdict[column['field']]) {
          let rg =
            (row[column['field']] - this.statdict[column['field']]['min']) /
            (this.statdict[column['field']]['max'] -
              this.statdict[column['field']]['min'])
        ...
        let rg =
          (this.statdict[column['field']]['max'] - row[column['field']]) /
          (this.statdict[column['field']]['max'] -
            this.statdict[column['field']]['min'])
        let clr = this.genColor(rg)
        return {
        ...
        column['field'] &&
        column['field'].startsWith('mean') &&
        this.statdict[column['field']]
      ) {
        let rg =
          (this.statdict[column['field']]['max'] - row[column['field']]) /
          (this.statdict[column['field']]['max'] -
    ...
    getProducts() {
      this.$axios
        .get('/fof/rank3', {
          params: { date: this.date, type: this.type, range: this.range }
        })
        .then((response) => {
          this.rgdict = {}
          this.subtypes = []
