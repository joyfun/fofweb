...
export default {
  ...
  methods: {
    ...
    getComanyPords(){
this.$axios
        .get("/fof/rank4", { params: { company_code:this.company_code,ftype:this.ftype,date: this.date,type:this.type ,range:this.srange,weight_type:this.filters['weight_type']} })
        .then((response) => {
      ...
      for (let a=0;a<tlen;){
        ...
        if(arow&&arow['company_code']&&cmap[arow['company_code']]){
          ...
          if(diff<=5){
            ...
            console.log(`####${arow['company_code']}:${old['code']}:${arow['code']}收益率差值${diff}超过5%不删除`)
            a++
          }
...
return ''
    ...
    compareInvest(){
          ...
          //   sels=this.tableList
          // }
          let holds=this.foflist.filter(row=>this.holding.filter(hd=>hd['b_code']==row['code']&&(ntype=='aas'||ntype==row['type'])).length>0).filter(row=>clss.indexOf( row['class_type'])>-1).map(row=>row['code'])
    ...
    getProducts() {
      ...
      this.$axios
        .get("/fof/rank3", { params: { ftype:this.ftype,date: this.date,type:this.type ,range:this.range,weight_type:this.filters['weight_type']} })
        .then((response) => {
          this.rgdict={}
          this.subtypes=[]
          ...
          this.tableData =response.data.map(row=>{
            ...
            for(var key of ['mean26','mean52','mean104','listrate26','listrate52','listrate104','std26','std52','std104']){
            if(row[key]){
      ...
      this.$axios
        .get("/fof/rankstat", { params: { ftype:this.ftype,date: this.date,type:this.type} })
        .then((response) => {
